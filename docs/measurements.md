...
**One pass, 2026-09-23.** Every number in this file was produced by a single measurement pass on the shipped `vseed` CLI on the date and machine below - except the `axe-heads` preset row in section 6, which was added on 2026-09-24 by the same protocol and says so where it stands, and the notes dated 2026-09-24 under "How each figure was taken" and in sections 6, 8.3 and 9, which record what the block-size change of that day altered and correct one false sentence; every measurement they sit beside is left as it was taken. Nothing here is copied from an earlier document or from a library-level benchmark.
...
**The one deviation from "as shipped": `--block-size 32`.** A wall budget is checked only when a worker is about to take a block, and a block already taken is always finished, so at the shipped default of 256 with 8 workers a 20 s budget can run on for a whole 256-seed block of one worker's time - `--budget 20s` on `all-traders` actually took **355 s** and evaluated 2,048 seeds, every worker having taken its first block before the budget ran out. At 32 the same command takes 46 s. The control: the same 240,000-seed G384 scan on 16 workers ran at 20,408.6 seeds/s at `--block-size 32` against 20,956.9 at 256 - a **2.62 %** cost, measured at the fastest tier, where per-block overhead matters most. Every preset above is therefore at most that much slower than its shipped-default self.

*Corrected 2026-09-24.* This paragraph used to say a wall budget "cannot stop a run sooner than one block per worker", so that 2,048 seeds was the smallest possible run. That floor does not exist: each worker builds its evaluator and only then looks at the budget, and a worker whose first look comes after it never takes a block at all - `--budget 0.001s --seeds 512` evaluated **0** seeds, and at `--block-size 16` a 0.02 s budget evaluated 32 seeds where that sentence said 128. What holds is an upper bound, one block of one worker's time past the budget with at most one block per busy worker in flight - plus the workers' start-up and the final write, which is why the 0-seed run above took 0.002 to 0.063 s over six runs against its 0.001 s, where one 16-seed block of it is about 0.03 s - and every budgeted run's plan now prints it with that run's numbers. The 355 s, 46 s and 2.62 % above are measurements and stand. The shipped default is no longer a fixed 256 either: left out, the block size is automatic (256, or smaller so every worker gets at least four blocks).
...
`--dry-run` is still not a promise - it is a single-thread loop over 8 seeds divided by `min(threads, 10.5)` (*2026-09-24:* its "the whole space" line still is; its "this run" line is now the busiest worker's seeds at the speed-up of the workers that have blocks, and its header prints the factor it used, where it used to say 10.5x at every thread count) - but on this build it is a reasonable first guess in both directions rather than a wild over-estimate. Confirm anything long with a short real run (`--seeds 20000`); a finished run's own `measured rate` is the real number.
...
1. **A wall budget can overrun by a whole block of one worker's time.** `vseed search all-traders --budget 20s` with the shipped `--block-size 256` and 8 workers ran for **355 s** and evaluated 2,048 seeds - one full round - because the budget is checked only when a worker takes a block, and a block already taken is finished. At `--block-size 32` the same command took 46 s. The budget bounds when the last block can *start*, not when the run ends, and nothing said so. *Corrected and fixed 2026-09-24:* this item used to call the budget a *floor-plus-one-round*; there is no floor (`--budget 0.001s` evaluates 0 seeds, and that run reported 100 % coverage until the same day). The plan now prints the overrun bound for every budgeted run, with its block size and the blocks in flight.
2. **Block size decides how many workers a short run actually uses.** Seeds per run divided by block size is the number of blocks; if that is below the worker count, the extra workers idle. At the shipped default a 540-seed G12 run is two blocks. This is correct behaviour for a long scan and a trap for anyone benchmarking with a short one - including this pass on its first attempt, where it turned a real 6.36x scaling at G12 into a measured 1.04x. *Fixed 2026-09-24:* left out, the block size is now automatic - 256, or smaller so every worker gets at least four blocks, which makes that 540-seed run 34 blocks of 16 on 8 workers - and the plan says so; a size that is given is kept and warned about when it leaves workers idle, and a rate measured with idle workers is labelled `(N of M workers had work)`.
